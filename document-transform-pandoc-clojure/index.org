...
So I decided to annotate the source instead.
My Clojure source viewer is as per 2022-07-15 not particularly happy with long lines.
Here's the raw file: [[file:rickroll.clj][rickroll.clj]].
Or you can view [[https://github.com/teodorlu/play.teod.eu/blob/master/document-transform-pandoc-clojure/rickroll.clj][rickroll.clj on Github]].

* Source for =rickroll.clj=
...
#+INCLUDE: "rickroll.sh" src shell-script
...
[[https://github.com/weavejester/hiccup][Hiccup]] is actually quite compact when you think about it.

#+begin_src clojure
[:p "See " [:a {:href "https://www.youtube.com/watch?v=dQw4w9WgXcQ"} "teod.eu"]]
#+end_src
* Let's rickroll ourselves!
#+begin_src
pandoc -i index.html --filter rickroll.sh -o rick.html
#+end_src

Please head over to [[rick.html][rick.html]]!
